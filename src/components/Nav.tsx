  ...
  return (
    <div class="bg-[#121120]">
      <header class="absolute inset-x-0 top-0 z-50">
        <nav class="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
          ...
          <div class="flex lg:hidden">
            ...
            >
              ...
              <HiOutlineBars3 class="h-6 w-6" aria-hidden="true" />
            </button>
          </div>
          <div class="hidden lg:flex lg:gap-x-12">
            {navigation.map((item) => (
              <A href={item.href} class={`text-sm font-semibold leading-6 text-white ${active(item.href)}`}>
                {item.name}
              </A>
            ))}
          </div>
        ...
        <Dialog as="div" class="lg:hidden" isOpen={mobileMenuOpen()} onChange={setMobileMenuOpen}>
          ...
          <DialogPanel class="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-gray-900 px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-white/10">
            <div class="flex items-center justify-between">
              ...
              >
                ...
                <HiOutlineXMark class="h-6 w-6" aria-hidden="true" />
              </button>
            </div>
